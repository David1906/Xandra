...
class QtExceptHook(object):
    ...
    :param reporting_cb: If provided, a :guilabel:`Report Error...` button will
    ...
    def __init__(self, reporting_cb: Callable[[str], None] = None):
        ...
        (Maximize the chance that a error in the exception handling system will
        ...
        if self._reporting_cb:
            btn_report = self._dialog.addButton(
                _tr("Report Error..."), QMessageBox.ActionRole
            )
            btn_report.clicked.disconnect()
            btn_report.clicked.connect(self._cb_report_error)
        else:
    ...
    @pyqtSlot()
    def _cb_report_error(self):
        """Qt slot for the :guilabel:`Report Error...` button."""
    ...
    def _excepthook(self, exc_type, exc_value, exc_traceback):
        ...
        # Construct the text of the error report
...
def make_email_sender(
    from_address: str = None, smtp_host: str = "localhost"
) -> Callable[[str], None]:
    ...
    def send_email(traceback_text: str):
        ...
        try:
            ...
            QMessageBox(
                ...
                _tr("SMTP Failure"),
                _tr(
                    "An error was encountered while attempting to send "
                    "your error report. Please submit it manually."
                ),
            ).exec_()
        else:
            QMessageBox(
                QMessageBox.Information,
                _tr("Error Reported"),
                _tr("Your error report was successfully sent."),
            ).exec_()
...
if __name__ == "__main__":
    ...
    reportOption = QCommandLineOption(
        "report-button",
        _tr("Initialize the exception hook with a error-reporting callback"),
    )
