...
class MoboFctHostControlDAO:
    ...
    def __new__(cls, *args, **kwargs):
        if not hasattr(cls, "instance"):
            cls.instance = super(MoboFctHostControlDAO, cls).__new__(
                cls, *args, **kwargs
            )
    ...
    def get_all_fixture_configs(self) -> "list[dict]":
        return self.data[MoboFctHostControlDAO.FIXTURES_ARRAY_KEY]

    def get_all_fixtures_ip(self) -> "list[str]":
        configs = self.get_all_fixture_configs()
        return [config[MoboFctHostControlDAO.PLC_IP_KEY] for config in configs]
